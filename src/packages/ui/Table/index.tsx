...
const Table = forwardRef<HTMLTableElement, TableProps>(({ className, border, children }, ref) => {
  return (
    <div
      className={cn('rounded-[6px] overflow-hidden box-border', {
        'border border-gray-200/80 [&_tr:last-child]:border-0': border,
      })}>
      <table ref={ref} className={cn('table-auto w-full border-collapse border-spacing-0', className)}>
        {children}
      </table>
    </div>
  );
...
const TableHeader = forwardRef<HTMLTableSectionElement, TableBaseType>(({ className, children }, ref) => {
  return (
    <thead ref={ref} className={cn('w-full bg-gray-200/30 border-b-[1px] border-gray-200/80 text-[14px]', className)}>
...
const TableHeaderCell = forwardRef<HTMLTableCellElement, TableBaseType>(({ className, children }, ref) => {
  return (
    <th ref={ref} className={cn('font-semibold p-cell text-start', className)}>
      {children}
    </th>
  );
});
